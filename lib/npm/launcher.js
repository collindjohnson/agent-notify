...
function runUnixCli(commandName, args) {
  const shellScript = path.join(repoRoot, 'bin', 'agent-notify');
  ensureFile(shellScript, `agent-notify runtime not found at ${shellScript}`);
  ...
  const result = spawnSync(
    ...
    [shellScript, ...args],
    {
      ...
      env: {
        ...process.env,
        AGENT_NOTIFY_COMMAND_NAME: commandName,
        AGENT_NOTIFY_INSTALL_METHOD: 'npm'
      }
    }
  );

  exitWithSpawnResult(result, 'bash is required to run agent-notify.');
}
...
function bootstrapWindowsRuntime() {
  ...
  const result = spawnWithCandidates(
    ...
    'PowerShell is required to bootstrap agent-notify on Windows.'
  );

  if (result.error || result.status !== 0) {
    exitWithSpawnResult(result, 'PowerShell is required to bootstrap agent-notify on Windows.');
  }
}

function runWindowsCli(commandName, args) {
  const runtimeScript = path.join(os.homedir(), '.agent-notify', 'bin', `${commandName}.ps1`);

  if (!fs.existsSync(runtimeScript)) {
    bootstrapWindowsRuntime();
  }

  ensureFile(runtimeScript, `agent-notify runtime not found at ${runtimeScript}`);
  ...
  const result = spawnWithCandidates(
    ...
    'PowerShell is required to run agent-notify on Windows.'
  );

  exitWithSpawnResult(result, 'PowerShell is required to run agent-notify on Windows.');
}
