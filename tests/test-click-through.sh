...
ROOT_DIR="$SCRIPT_DIR/.."
NOTIFIER="$ROOT_DIR/lib/agent-notify/core/notifier.sh"
...
mkdir -p "$HOME/.agent-notify" "$HOME/.claude/notifications" "$HOME/.claude/logs" "$fake_bin" "$fake_app/Contents" "$phpstorm_app/Contents"
...
status_before=$(HOME="$HOME" "$ROOT_DIR/bin/agent-notify" click-through status 2>&1 || true)
printf '%s' "$status_before" | grep -q "click-through add" || fail "status should guide users to add a mapping when none exists"

add_output=$(printf 'fake_term\n' | HOME="$HOME" "$ROOT_DIR/bin/agent-notify" click-through add "$fake_app" 2>&1)
...
repeat_add_output=$(
    ...
    AGENT_NOTIFY_CLICK_THROUGH_APP_PATH="$fake_app" \
    "$ROOT_DIR/bin/agent-notify" click-through add 2>&1
)
...
status_after=$(HOME="$HOME" "$ROOT_DIR/bin/agent-notify" click-through status 2>&1)
...
remove_output=$(printf ' \n' | HOME="$HOME" "$ROOT_DIR/bin/agent-notify" click-through remove 2>&1)
...
status_final=$(HOME="$HOME" "$ROOT_DIR/bin/agent-notify" click-through status 2>&1 || true)
...
HOME="$HOME" \
TERM_PROGRAM="" \
AGENT_NOTIFY_CLICK_THROUGH_APP_PATH="$phpstorm_app" \
bash "$NOTIFIER" test >/dev/null 2>&1
...
jetbrains_add_output=$(
    ...
    "$ROOT_DIR/bin/agent-notify" click-through add FakeCodex 2>&1
)
...
cancel_output=$(printf 'q' | HOME="$HOME" "$ROOT_DIR/bin/agent-notify" click-through remove 2>&1 || true)
